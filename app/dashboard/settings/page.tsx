...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      ...
      setApiAccessEnabled(settings.apiAccessEnabled);
      setApiRateLimits(settings.apiRateLimits);
      setIsLoading(false);
    };
  ...
  const handleSaveSettings = async (section: string) => {
    const payloadBySection: Record<string, Record<string, unknown>> = {
      general: {
        companyName,
        companyEmail,
        companyWebsite,
    ...
    if (!response.ok || !response.data?.settings) {
      toast({
        ...
        description:
          typeof response.error === "string"
            ? response.error
            : `Unable to save your ${section} settings.`,
        variant: "destructive",
      });
      return;
    }
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className="bg-slate-50 border border-slate-200 text-slate-900 dark:bg-[#1a1625] dark:border-0 dark:text-white focus-visible:ring-purple-500 max-w-xs"
                      min="5"
                      max="120"
                    />
                    <p className="text-sm text-slate-500 dark:text-gray-400">
            ...
            <Card className="bg-white border-slate-200 text-slate-900 dark:bg-[#231c35] dark:border-[#2a2139] dark:text-white">
              ...
              <CardContent className="space-y-6">
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <span className="text-sm font-medium">
                              {apiRateLimits.perDay.toLocaleString()}
                            </span>
                          </div>
                        </div>
                      </div>
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {savingSection === "api" ? "Saving..." : "Save Changes"}
                  </Button>
                </div>
              </CardContent>
            </Card>
